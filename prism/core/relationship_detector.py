...
class RelationshipDetectionEngine:
    ...
    def __init__(self):
        ...
        # Initialize age-appropriate context detector
        self.age_detector = AgeDetector()
        logger.info("Age detection engine loaded successfully")
        
    def analyze_relationships(self, image_path: str, detected_objects: List[Dict], 
                            scene_context: Dict = None, caption: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Determine age-appropriate context using real scene and caption data
            object_names = [obj['name'] for obj in detected_objects]
            scene_type = scene_context.get('type', 'unknown') if scene_context else 'unknown'
            
            # Use caption for educational context detection if available
            combined_context = f"{scene_type} {caption or ''}"
            
            age_context = self.age_detector.analyze_age_context(
                combined_context, object_names, features['people_count']
            )
            
            # Debug age context detection
            logger.info(f"Age context analysis: {age_context}")
            logger.info(f"Scene type: {features.get('scene_type', 'unknown')}")
            logger.info(f"Objects detected: {object_names}")
            
            # Analyze spatial relationships  
            spatial_analysis = self._analyze_spatial_dynamics(features, age_context)
            ...
            # Revolutionary individual person profiling (age-appropriate)
            individual_profiles = self.person_profiler.profile_individuals(
                features['people_count'], detected_objects, spatial_analysis, power_structure, age_context
            )
    ...
    def _extract_visual_features(self, image_path: str, detected_objects: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'scene_type': 'unknown'  # Will be updated by analyzer
        }
    
    def _analyze_spatial_dynamics(self, features: Dict[str, Any], age_context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        is_educational = age_context and age_context.get('educational_setting', False)
        
        if is_educational:
            # Educational context dynamics
            if people_count <= 5:
                return {
                    'type': 'small_learning_group',
                    'formation': 'interactive_learning',
                    'leadership_pattern': 'teacher_guided',
                    'engagement_level': 'educational'
                }
            else:
                return {
                    'type': 'classroom',
                    'formation': 'instructor_centered',
                    'leadership_pattern': 'educational_authority',
                    'engagement_level': 'student_focused'
                }
        
        # Corporate/business context dynamics
        elif features['business_context']:
            if people_count <= 3:
                return {
                    'type': 'small_meeting',
