...
class PersonProfiler:
    ...
    def profile_individuals(self, people_count: int, objects: List[Dict], 
                          spatial_analysis: Dict, power_dynamics: Dict, age_context: Dict = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Determine roles based on positioning and context (age-appropriate)
            role_assignments = self._assign_appropriate_roles(
                people_count, spatial_positions, power_dynamics, age_context
            )
    ...
    def _assign_appropriate_roles(self, people_count: int, positions: Dict, 
                                 power_dynamics: Dict, age_context: Dict = None) -> Dict[str, str]:
        """Assign age-appropriate roles based on positioning and context"""
        ...
        if is_educational:
            # Use educational roles
            appropriate_roles = age_context.get('appropriate_roles', {})
            position_keys = list(positions.keys())
            
            # First person is likely the teacher/authority figure
            if len(position_keys) > 0:
                role_assignments[position_keys[0]] = random.choice(appropriate_roles.get('authority_figure', ['Teacher']))
            
            # Remaining people are students/participants
            for i, person_key in enumerate(position_keys[1:], 1):
                if i < len(position_keys):
                    role_assignments[person_key] = random.choice(appropriate_roles.get('participants', ['Student']))
            
            return role_assignments
        
        # Corporate context (original logic)
